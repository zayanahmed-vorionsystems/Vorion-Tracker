...
const MAX_FILE_BYTES = 8 * 1024 * 1024; // 8MB safety cap for a single screenshot
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // employeeId comes from the authenticated token, never from the request
    // body — trusting a client-supplied employeeId would let a compromised
    // agent attribute screenshots to any employee.
    ...
    const saved = await withTransaction(async (client) => {
      ...
      // Screenshots prove the agent is connected, not that the user is
      // actively working — the agent keeps capturing while idle. Preserve
      // whatever status the heartbeat last reported instead of forcing
      // "working" here; only seed a row on first insert.
