...
if (globalScope.__worktrackCaptureRendererInitialized) {
  ...
  globalScope.__worktrackLivePublisher = globalScope.livePublisher;

  if (!globalScope.livePublisher) {
    console.error('[AGENT][LIVEKIT] preload bridge missing');
  } else {
    globalScope.livePublisher.onStart((payload) => {
      startPublishing(payload).catch((error) => {
        log({ state: 'start-failed', message: error instanceof Error ? error.message : String(error) });
      });
    });

    globalScope.livePublisher.onStop(() => {
      void stopPublishing();
    });

    globalScope.livePublisher.sendReady();
    log({ state: 'ready' });
  }
}